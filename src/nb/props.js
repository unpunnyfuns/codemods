 ...
 * - nativebase-props.js: Source model (NativeBase props documentation)
 */
...
function getStyleInvalidReason(styleName, value) {
  // Check if prop is not supported on View at all
  if (!VIEW_STYLE_PROPS[styleName]) {
    // Text-only props
    if (
      [
  ...
  if (dimensionProps.includes(styleName) && validSpaceTokens.includes(value.replace(/"/g, ''))) {
    return 'Semantic token not valid for dimension prop (use numeric value)'
  }

  return 'Invalid value for this prop'
}
