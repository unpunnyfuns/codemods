 ...
 *
 * ARCHITECTURE:
 * Utility codemod that removes unused imports and variable declarations.
 * Uses multi-pass strategy to handle cascading removals (removing A makes B unused).
 * Supports both runtime JavaScript and TypeScript type usage detection.
 *
 * USAGE:
 * ./run.sh remove-unused "src/STAR_STAR/STAR.tsx"
 * ./run.sh remove-unused "src/STAR_STAR/STAR.tsx" --verbose
 * ./run.sh remove-unused "src/STAR_STAR/STAR.tsx" --imports=false (only remove variables)
 * ./run.sh remove-unused "src/STAR_STAR/STAR.tsx" --variables=false (only remove imports)
 * (Replace STAR_STAR with two asterisks, STAR with one asterisk)
 *
 * OPTIONS:
 * - imports (default true): Remove unused imports
 * - variables (default true): Remove unused variable declarations
 * - verbose (default false): Log removed identifiers to console
 *
 * REMOVAL STRATEGY:
 * 1. Find all identifier usages in file (runtime and TypeScript types)
 * 2. Remove unused imports
 * 3. Remove unused variable declarations
 * 4. Repeat until no changes (cascading removals)
 * 5. Maximum 10 passes (safety limit)
 *
 * RUNTIME USAGE DETECTION:
 * Tracks identifiers in:
 * Variable references
 * Function calls
 * JSX elements and props
 * Object properties (including shorthand)
 * Export statements
 *
 * TYPESCRIPT TYPE USAGE DETECTION:
 * Tracks type names in:
 * Type annotations: const x: Type
 * Type parameters: useForm<Type>()
 * Type aliases: type Foo = Bar
 * Union/intersection types: Type1 | Type2
 * Qualified names: Namespace.Type
 * Generic parameters: Array<Type>
 * Indexed access: Props['variant']
 * Type queries: typeof Type
 *
 * PROTECTED DECLARATIONS:
 * Never removes:
 * Exported declarations (export const, export { ... })
 * Default exports (export default)
 * Side-effect imports (import './styles')
 *
 * CASCADING REMOVALS:
 * Multi-pass handles chains:
 * Pass 1: remove unused import B (only used by variable A)
 * Pass 2: remove variable A (now unused after B removed)
 * Pass 3: no changes (done)
 *
 * USE CASES:
 * - Cleanup after component migrations (remove old imports)
 * - Remove dead code after refactoring
 * - Clean up unused TypeScript types
 * - Prepare for final code review (remove clutter)
 *
 * See remove-unused.md for detailed documentation.
 */
