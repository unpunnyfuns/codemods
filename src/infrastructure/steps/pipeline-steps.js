...
import { findJSXElements } from '@puns/shiftkit/jsx'
...
export function manageImports(elementName, customManager = null) {
  return (ctx) => {
    if (customManager) {
      // Use custom import management
      customManager(ctx)
      return { ctx }
    }

    // Standard import management
    ...
    // Add target import (only if not already present and we didn't keep the source)
